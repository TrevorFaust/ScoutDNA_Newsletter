...
#
# Default: Reddit + RSS + YouTube + podcasts + camp signals every day. Weekly LLM
# compose runs after the 3am PT nflverse sync on Tuesdays, not in this 1am job.
#
...
Register-ScheduledTask `
    ...
    -Trigger $Trigger `
    -Settings $Settings `
    -Description "ScoutDNA daily pipeline: collect + media every day (Sunday included). Weekly compose is the 3am nflverse task on Tuesday." `
    -Force | Out-Null
...
Write-Host "  Pipeline: Reddit + RSS$(if (-not $SkipMedia) { ' + YouTube + podcasts' })$(if (-not $SkipCompose) { ' (weekly compose is the 3am Tuesday nflverse task)' } else { ' (no compose, ever)' })"
Write-Host ""
